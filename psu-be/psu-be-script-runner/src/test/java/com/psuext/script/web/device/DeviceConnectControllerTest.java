...
class DeviceConnectControllerTest extends DeviceManagerControllerTestSupport {

    @Test
    void connectsDeviceById() {
        connect(DEVICE_ID);
        awaitConnected();
    }

    @Test
    void connectsDeviceByName() {
        connect(DEVICE_NAME);
        awaitConnected();
    }

    @Test
    void connectMissingDeviceReturnsNotFound() {
        restClient.post().uri("/api/devices/MISSING/connect")
